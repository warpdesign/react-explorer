...
class DownloadsClass extends React.Component<Props, State> {
    ...
    private installReaction(): void {
        this.disposer = reaction(
            (): IObservableArray<Batch> => toJS(this.appState.transfers),
            ...
            for (const element of transfer.elements) {
                if (!element.file.isDir || element.status === 'error') {
                    ...
                    node.childNodes.push({
                        id: id,
                        icon: element.file.isDir ? 'folder-close' : this.getFileIcon(filetype),
                        label: element.subDirectory
                            ? element.subDirectory + sep + element.file.fullname
