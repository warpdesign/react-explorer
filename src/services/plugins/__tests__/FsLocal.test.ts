...
import { describeUnix, describeWin, itUnix, getPath } from '../../../utils/test/helpers'
...
describe('FsLocal', () => {
    describeUnix('isDirectoryNameValid', function () {
        beforeAll(() => {
            localAPI = new FsLocal.API('/', jest.fn())
        })
    ...
    describe('list', () => {
        beforeAll(() => {
            localAPI = new FsLocal.API('/', jest.fn())
            jest.spyOn(localAPI, 'onList').mockImplementation(jest.fn())
            mock(TmpDir)
        })
    ...
    describeWin('isRoot', () => {
        beforeAll(() => {
            localAPI = new FsLocal.API('C:\\', jest.fn())
        })
