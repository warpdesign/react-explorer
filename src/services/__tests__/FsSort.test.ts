...
describe('sorting methods', () => {
    it('sort by Name/Asc', () => {
        ...
        const sortedIds = files.sort(sortMethod).map((file) => file.id.ino);
        expect(sortedIds).toEqual([2, 0, 1, 3]);
    });

    it('sort by Name/Desc', () => {
        const sortMethod = getSortMethod('name', 'desc');
        const sortedIds = files.sort(sortMethod).map((file) => file.id.ino);
        expect(sortedIds).toEqual([3, 1, 0, 2]);
    });

    it('sort by Size/Asc', () => {
        const sortMethod = getSortMethod('size', 'asc');
        const sortedIds = files.sort(sortMethod).map((file) => file.id.ino);
        expect(sortedIds).toEqual([1, 0, 3, 2]);
    });

    it('sort by Size/Asc', () => {
        const sortMethod = getSortMethod('size', 'desc');
        const sortedIds = files.sort(sortMethod).map((file) => file.id.ino);
        expect(sortedIds).toEqual([2, 3, 0, 1]);
    });
});
