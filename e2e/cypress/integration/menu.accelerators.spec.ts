...
describe('combo hotkeys', () => {
    let caches: FileState[];

    function resetSelection(): void {
        cy.window().then((win) => {
            win.appState.winStates[0].views.forEach((view: ViewState) => {
                view.caches.forEach((cache: FileState) => {
                    cache.reset();
                });
            });
        });
    ...
    function createStubs(): Chainable<Window> {
        cy.window().then((win) => {
            const { appState } = win;
